...
$("#greeting").textContent=`${timeGreeting}، سعادة الأمين العام`;
...
$("#addContact").onclick=()=>{
  $("#simpleForm").innerHTML=`<button class="close" value="cancel">×</button><p class="eyebrow">دفتر الاتصالات</p><h2>جهة اتصال جديدة</h2><input name="name" required placeholder="الاسم الكامل"><input name="role" placeholder="المنصب"><input name="org" placeholder="الجهة أو الإدارة"><input name="phone" type="tel" inputmode="tel" placeholder="رقم الهاتف"><small class="voice-hint">يمكنك قول الرقم بالصوت، ثم مراجعته قبل الحفظ.</small><input name="email" type="email" inputmode="email" placeholder="البريد الإلكتروني"><small class="voice-hint">قل مثلًا: name آت example نقطة com</small><textarea name="notes" rows="4" placeholder="ملاحظات الاتصال"></textarea><div class="modal-actions"><button value="cancel">إلغاء</button><button type="submit" class="primary">حفظ جهة الاتصال</button></div>`;
...
const audioDb={
  db:null,
  async init(){return new Promise((res,rej)=>{const r=indexedDB.open("moeen_audio",1);r.onupgradeneeded=()=>r.result.createObjectStore("audio");r.onsuccess=()=>{this.db=r.result;res()};r.onerror=()=>rej(r.error)})},
  ...
  async put(k,blob){
    ...
    try{await fetch(`/api/sync/audio/${encodeURIComponent(k)}`,{method:"PUT",headers:{"Content-Type":"application/octet-stream","X-CSRF-Token":apiCsrf,"X-Audio-IV":record.iv,"X-Audio-Type":record.type},body:cipher})}catch{}
  },
  async get(k){
    ...
    if(!record){
      try{const r=await fetch(`/api/sync/audio/${encodeURIComponent(k)}`,{headers:{"X-CSRF-Token":apiCsrf}});if(!r.ok)return null;record={cipher:await r.arrayBuffer(),iv:r.headers.get("X-Audio-IV"),type:r.headers.get("X-Audio-Type")||"audio/webm"};await this.rawPut(k,record)}catch{return null}
    }
...
async function initializeOrUnlockVault(password,info){
...
function legacyState(){
  ...
  dataKinds.forEach(k=>{try{state[k]=JSON.parse(localStorage.getItem("moeen_"+k)||"[]").map(x=>({...x,_updated:x._updated||stamp}))}catch{state[k]=[]}});
  return state;
}
function hasData(state){return dataKinds.some(k=>(state[k]||[]).length)}
async function loadAndSyncState(){
  let local=emptyState(),cache=localStorage.getItem("moeen_secure_cache");
...
async function saveEncryptedCache(){
  if(!vaultKey)return;
  const payload=await encryptObject(secureState);localStorage.setItem("moeen_secure_cache",JSON.stringify({...payload,version:syncVersion}));
}
...
async function pushSync(retry=true){
  ...
  try{
    ...
    dataKinds.forEach(k=>localStorage.removeItem("moeen_"+k));
...
function deviceId(){let id=localStorage.getItem("moeen_device_id");if(!id){id=crypto.randomUUID();localStorage.setItem("moeen_device_id",id)}return id}
function deviceName(){const mobile=/iPhone|iPad|Android/i.test(navigator.userAgent);return mobile?"هاتف الأمين العام":"لابتوب الأمين العام"}
async function initAuth(){
  try{
    const status=await api("/api/auth/status");
    if(status.authenticated)apiCsrf=status.csrf||"";
...
$("#loginForm").onsubmit=async e=>{e.preventDefault();const password=$("#loginPassword").value,payload={password,device_id:deviceId(),device_name:deviceName()},code=$("#pairingCode").value.trim();try{const result=code?await api("/api/auth/pair",{method:"POST",body:JSON.stringify({...payload,pairing_code:code})}):await api("/api/auth/login",{method:"POST",body:JSON.stringify(payload)});apiCsrf=result.csrf||apiCsrf;await initializeOrUnlockVault(password,result.vault);document.body.classList.remove("locked");$("#authMessage").textContent="";$("#loginPassword").value="";if(result.must_change){setView("security");alert("يرجى تغيير كلمة المرور المؤقتة.")}}catch(err){const messages={INVALID_CREDENTIALS:"كلمة المرور غير صحيحة.",DEVICE_NOT_AUTHORIZED:"هذا الجهاز غير مصرح له. استخدم رمز ربط من الجهاز الرئيسي.",INVALID_PAIRING_CODE:"رمز الربط غير صحيح أو انتهت صلاحيته.",TEMPORARILY_BLOCKED:"تم حظر المحاولات مؤقتًا. حاول لاحقًا.",OperationError:"تعذر فتح الخزنة. تحقق من كلمة المرور."};$("#authMessage").textContent=messages[err.message]||"تعذر تسجيل الدخول أو فتح الخزنة."}};
async function logoutNow(){try{await pushSync();await api("/api/auth/logout",{method:"POST",body:"{}"})}finally{apiCsrf="";vaultKey=null;secureState=emptyState();document.body.classList.add("locked");$("#loginPane").hidden=false;$("#loginPassword").value="";$("#pairingCode").value=""}}

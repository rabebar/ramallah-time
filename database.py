...
from sqlalchemy import (
    ...
    Text, Boolean, DateTime, Float, ForeignKey
)
from sqlalchemy.orm import declarative_base, sessionmaker, relationship

# إعدادات المسارات والقاعدة
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_FILENAME = os.getenv("DB_FILENAME", "ramallah_final.db")
DB_PATH = os.path.join(BASE_DIR, DB_FILENAME)

# جلب رابط قاعدة البيانات (يدعم SQLite محلياً و Postgres على Render)
DATABASE_URL = os.getenv("DATABASE_URL", f"sqlite:///{DB_PATH}")

# تصحيح رابط Postgres الخاص بـ Render
...
class Place(Base):
    ...
    # حساب المالك (تم تغيير String(100) إلى Text لضمان التشفير)
    owner_email = Column(String(255), nullable=True, unique=True, index=True)
    owner_password = Column(Text, nullable=True) 
    ...
    images = relationship("PlaceImage", back_populates="place", cascade="all, delete-orphan", lazy="selectin")
